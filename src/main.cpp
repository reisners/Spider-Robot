...
void stand();
void step_forward(unsigned int n);
void step_back(unsigned int n);
void turn_left(unsigned int n);
void turn_right(unsigned int n);
void hand_shake(int n);
void hand_wave(int n);
void sit();
...
int next_int(SerialCommands* sender, const char *error_msg) {
	char* str = sender->Next();
	if (str == NULL)
	{
...
float next_float(SerialCommands* sender, const char *error_msg) {
	char* str = sender->Next();
	if (str == NULL)
	{
		sender->GetSerial()->println(error_msg);
...
void cmd_stand(SerialCommands* sender) {
  stand();
  sender->GetSerial()->println("stand executed");
}
SerialCommand cmd_stand_("u", cmd_stand);

void cmd_forward(SerialCommands* sender) {
	int n = next_int(sender, 1);
  step_forward(n);
  sender->GetSerial()->println("step_forward executed");
}
SerialCommand cmd_forward_("f", cmd_forward);

void cmd_back(SerialCommands* sender) {
	int n = next_int(sender, 1);
  step_back(n);
  sender->GetSerial()->println("step_back executed");
}
SerialCommand cmd_back_("b", cmd_back);

void cmd_left(SerialCommands* sender) {
	int n = next_int(sender, 1);
  turn_left(n);
  sender->GetSerial()->println("turn_left executed");
}
SerialCommand cmd_left_("l", cmd_left);

void cmd_right(SerialCommands* sender) {
	int n = next_int(sender, 1);
  turn_right(n);
  sender->GetSerial()->println("turn_right executed");
}
SerialCommand cmd_right_("r", cmd_right);

void cmd_wave(SerialCommands* sender) {
	int n = next_int(sender, 1);
  hand_wave(n);
  sender->GetSerial()->println("hand_wave executed");
}
SerialCommand cmd_wave_("w", cmd_wave);

void cmd_shake(SerialCommands* sender) {
	int n = next_int(sender, 1);
  hand_shake(n);
  sender->GetSerial()->println("hand_shake executed");
}
SerialCommand cmd_shake_("s", cmd_shake);

void cmd_sit(SerialCommands* sender) {
  sit();
  sender->GetSerial()->println("sit executed");
}
SerialCommand cmd_sit_("d", cmd_sit);
...
int servo_angle_correction[4][3] = { { 0, 0, 0 }, { 0, -3, 6 }, { 7, -10, -4 }, { -7, -7, -4 } };
/* Size of the robot ---------------------------------------------------------*/
...
void setup()
{
  ...
  serial_commands_.AddCommand(&cmd_right_);
...
void polar_to_servo(int leg, float alpha, float beta, float gamma)
{
  ...
  servo[leg][0].write(alpha + servo_angle_correction[leg][0]);
  servo[leg][1].write(beta + servo_angle_correction[leg][1]);
  servo[leg][2].write(gamma + servo_angle_correction[leg][2]);
}
